...
public class Produce_Consume {
	...
	private volatile boolean csv_updated = false;
    String accessToken;
	String nxt_link;
    String url = "https://graph.microsoft.com/v1.0/users";
	
	public Produce_Consume(String nxt_link)
	{
		this.nxt_link=nxt_link;
	}
        ...
        try {
            ...
            if (httpResponseCode == HttpURLConnection.HTTP_OK) {
                try (BufferedReader in = new BufferedReader(new InputStreamReader(conn.getInputStream()))) {
                    ...
                    if (users.isEmpty()|| !jsonResponse.has(nxt_link)) {
                        is_last=true;
						update_csv(users);
					...
					update_csv(users);
                    url = jsonResponse.getString(nxt_link);
                }
            }
        }} catch (Exception e) {}
    }
	
	public void update_csv(JSONArray users)
	{
		//Append the data to csv code 
		...
		count_users++;
		//System.out.println("Hundreds : "+count_users);
		//System.out.println("CSV Update : "+this.csv_updated);
		if(is_last)
		{
			csv_updated=true;
		}
		if(count_users==50)//if 5000 users are produced
		{
			current_csv++;
			csv_updated=true;
			count_users=0;
    ...
    public void consume() 
	{
		if(csv_updated==true)
		{
			if(current_csv==1)
			{
				System.out.println("Incrementing current csv");
				current_csv++;
			}
